...
export default function Container() {
	...
	const handleCloneInput = () => {
		...
		const newInput = { id: lastInput.id + 1, value: '' };
		setInputs([...inputs, newInput]);
	};

	const handleDrop = (status) => (item) => {
		console.log('updatedInput::drag', JSON.stringify(item));
		const input = item.input;
		if (status === 'firstInput') {
			const updatedInput = [...firstInputs, input];
			setTodoInputs(updatedInput);
			console.log('updatedInput: ', updatedInput);
		} else if (status === 'secondInput') {
			setInprogressInputs([...secondInputs, input]);
		}
	};
	...
	return (
		<div className='flex align-baseline bg-slate-300 h-[100vh]'>
			...
			<div className='flex justify-between gap-3 p-4'>
				<div>
					<div className='flex p-4 items-center'>
						...
						<select
							className='w-[100px] p-2 border rounded m-3'
...
const DropArea = ({ status, inputs, onDrop }) => {
	console.log('inputs onDrp: ', inputs);
